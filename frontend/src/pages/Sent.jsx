...
const Sent = () => {
  ...
  useEffect(() => {
    if (user) {
      setLoading(true);
      axios
        .get("http://localhost:5000/emails/sent", { withCredentials: true })
        .then((response) => {
  ...
  const speakEmail = (email) => {
    ...
    try {
      // Speak metadata
      const metaText = `Email sent to ${email.to}. Subject: ${email.subject}`;
  ...
  // Stop speech recognition for navigation
  const stopListening = () => {
  ...
  // Voice-enabled search input component
  const SearchInput = () => (
        ...
        <div className="max-w-6xl mx-auto">
          ...
          {!selectedEmail ? (
            ...
            <div className="bg-white shadow-sm rounded-lg p-6">
              ...
              <p className="text-gray-600 mt-2">To: {selectedEmail.to}</p>
